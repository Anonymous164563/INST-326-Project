...
class GraphingCalculatorApp:
    ...
    def ibiza_sunset(self):
        """Creates th background for the interactive calculator app """
    ...
    def _draw_gradient(self, width, height):
        """Creates and renders the background for the application"""
    ...
    def _redraw_gradient(self, event):
        """In case the app is refreshed, the graidnet is then  redrawn in here"""
    ...
    def _lower_background(self):
        """Renders the whole entire gradient"""
        self.gradient.lower("all")

    # ------------------- INPUTS -------------------
    def _create_expression_entry(self, root):
        """Creates the box for entering the mathemical expression"""
    ...
    def _create_x_value_entry(self, root):
        """Enables the user to enter the x values"""
    ...
    def _create_buttons(self, root):
        """Creates the interactive buttons for the user to pres"""
    ...
    def _create_numeric_and_operator_buttons(self, button_frame):
        """ Creates the button layout as an array"""
        buttons = [
    ...
    def _create_graph_area(self, root):
        """Creates the area for the graph to view using Figure and FigureCanvasTkAgg as the acceptibel methods"""
    ...
    def _on_button_click(self, value):
        """When the equal sign is clicked the functions related to calculating the value will be called"""
        if value == "=":
            self._on_calc_value()
        else:
    ...
    def graph_calculations(self):
        """Evlauetes the epxression for the graph, then draws the graph"""
    ...
    def _on_calc_value(self):
        """Gets the epxpressions and x values entered"""
    ...
    def _3D_Render_Callback(self):
        """Gets the expression entered first, when there is no expression enetered, disp;ay a error message that the image was not able to 
        be render"""
        expression = self.expression_entry.get()
    ...
    def three_dimension_Render(self, expression):
        """Attepmts to render the 3D equivalent of the expression, if not an error will be rendered"""
        import numpy as np
        import sympy as sym
    ...
    def three_dim_animate(self):
        """Attempts to render the 3D image"""
        import numpy as np
        from matplotlib.animation import FuncAnimation
        from mpl_toolkits.mplot3d import Axes3D  # noqa: F401

        # Clear figure and set 3d axis
        self.plot_area.figure.clf()
        ax = self.plot_area.figure.add_subplot(111, projection="3d")
        self.plot_area = ax
        ...
        if hasattr(self, 'ani'):
            self.ani.event_source.stop()  # Stop previous animation if running
